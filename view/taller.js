...
app.get("/ParticipantesTaller",async function (req, res)
{
    try {
        var datos = await ControllerTaller.readControllerParticipantesTaller()

        res.status(200).json({
            status_code: datos.length > 0 ? 200 : 300,
            msm : datos.length > 0 ? 'PARTICIPANTES DISPONIBLES' : 'NO EXISTEN PARTICIPANTES DISPONIBLES',
            datos : datos
        })
    }catch (e) {
        res.status(200).json({
...
app.get("/detalleTaller/:taller",async function (req, res)
{
    try {
        var datos = await ControllerTaller.readControllerDetalleTaller(req.params.taller)
